...
#define i386CopyMemory(src, dest, length) \
    asm volatile("cld\n\t" \
                 "rep movsb" :: "S"(src), "D"(dest), "c"(length));

#define i386CopyMemoryD(src, dest, length) \
    asm volatile("cld\n\t" \
                 "rep movsl" :: "S"(src), "D"(dest), "c"(length));
