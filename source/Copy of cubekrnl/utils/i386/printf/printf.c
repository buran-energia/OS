 ...
 */
static char* _makestr(char* buffer, char* end, long long num, int base, int precision, int *flags)
{
    ...
    while(*format)
    {
		...
		*format++; // next char
		...
		switch(*format)
		{
			...
			case 'p': // pointer
				if (minlen == -1)
					minlen = 2*sizeof(void*);
				FLAGS |= 0x1;
				FLAGS &= 0xef;
				FLAGS |= LARGE;
				_makestr(temp, end,
						(unsigned long) va_arg(args, void*), 16, precision, &FLAGS);
		...
		*format++; // next char befor continue
...
int sprintf(char* buffer, const char* format, ...)
{
	...
	va_start(args, format);
	i = vsnprintf(buffer, 0xFFFFFFFFUL, format, args);
	va_end(args);

	return i;
}

/* printf moved to stdio.c */
